...
for(i in 1:size)
{
  for(j in 1:size)
  {
    if(heatmaplattice[i,j]>b)
    {numofreddots=numofreddots+1
    par(new=T)
    polygon(c(j-1+0.2,j-0.2,j-0.2,j-1+0.2),c(size-i+0.2,size-i+0.2,size-i+1-0.2,size-i+1-0.2), density = NULL, border = F, col = 'red')
    }
  }
}
numofreddots/size/size
